...
* town (String) - A kikötő városának neve (nem lehet üres)
...
* berths (List&lt;Berth&gt;) - A kikötőkhöz kapcsolódó kikötőhelyek listája 
...
| ------------ | ----------------------- | ---------------------------------------------------------------------- |
| GET          | `"/api/ports"`        | Lekérdezi az összes Kikötő entitást. Az eredmény minden esetben ABC szerint lesz rendezve. Két opcionális paramétert lehet megadni: a `town` paraméterrel a kikötő városának nevére lehet szűrni, a `capacity` paraméterrel pedig a kikötőkben található minimális vengéghelyek számára.
| GET          | `"/api/ports/{id}"`   | Lekérdez egy Kikötő entitást `id` alapján. Ha a kikötő nem létezik, 404-es Status kóddal tér vissza..                                   |
  | POST        | `"/api/ports/"`   | Létrehoz egy Kikötő entitást                                 |
  | POST         | `"/api/ports/{id}"`   | Hozzáad egy új Kikötőhely entitást az `id`-vel rendelkező Kikötőhöz. Ha a kikötő nem létezik, 404-es Status kóddal tér vissza.        |
  | PUT         | `"/api/ports/{id}"`   | Módosít egy Kikötő entitást `id` alapján.  Ha a kikötő nem létezik, 404-es Status kóddal tér vissza.                                     |
  | DELETE         | `"/api/ports/{id}"`   | Töröl egy Kikötő entitást `id` alapján.  Ha a kikötő nem létezik, 404-es Status kóddal tér vissza.                                      |

Üzleti logika két funkció megvalósításához tartozik.
1. A kikötő vendéghelyeinek számát csak olyan szintre lehetséges csökkenteni, hogy az a már foglalásokkal rendelkező kikötőhelyek száma alá ne mehessen. Ha a felhasználó kevesebbre szeretné állítani a vendéghelyek számát, mint ami lehetséges, akkor RequestedNumberOfGuestBerthsIsLessThanBookedBerthsException generálódik, és az alkalmazás 400-as Status kóddal tér vissza. 
2. Új kikötőhely létrehozásakor figyelembe veszi, hogy az adott kikötőben mennyi lehet az engedélyezett vendég helyek maximális száma. Ha a felhasználó több vendéghelyet szeretne létrehozni, mint ami lehetséges, akkor NumberOfBerthsExceedsLimitException generálódik, és az alkalmazás 400-as Status kóddal tér vissza. 
...
| HTTP metódus | Végpont                 | Leírás                                                                 |
| ------------ | ----------------------- | ---------------------------------------------------------------------- |
| GET          | `"/api/berths"`        | Lekérdezi az összes Kikötőhely entitást. Az eredmény minden esetben a foglalások kezdeti dátuma szerint lesz rendezve. Két opcionális paramétert lehet megadni: a `town` paraméterrel a kikötő városának nevére lehet szűrni, a `width` paraméterrel pedig a kikötőhelyek minimális szélességére.                                 |
| GET          | `"/api/berths/{id}"`   | Lekérdez egy Kikötőhely entitást `id` alapján.  Az eredmény a foglalások kezdeti dátuma szerint lesz rendezve. Ha a kikötőhely nem létezik,  404-es Status kóddal tér vissza.                        |
  | POST         | `"/api/berths/{id}"`   | Hozzáad egy Booking-ot az `id`-val rendelkező kikötőhelyhez. Ha a kikötőhely nem létezik,  404-es Status kóddal tér vissza.           |
  | PUT         | `"/api/berths/{id}/bookings"`   | Módosít egy Kikötőhely entitást `id` alapján. Ha a kikötőhely nem létezik, 404-es Status kóddal tér vissza.                    |
  | DELETE         | `"/api/berths/{id}"`   | Töröl egy Kikötőhely entitást `id` alapján. Ha a kikötőhely nem létezik, 404-es Status kóddal tér vissza.                         |

Üzleti logika a foglalások létrehozásához tartozik:

1. Foglalás rögzítése akkor lehetéges, ha az az aktuális év szezonjára vonatkozik, amely alapesetben minden év 04.01-10.31-ig tart (egy Season objektum segítségével váltotatható).  Ha nem teljesül a feltétel, akkor OutOfSeasonsException generálódik, és az alkalmazás 400-as Status kóddal tér vissza.
2. Foglalás olyan napokra lehetséges, amelyekre korábban nem érkezett foglalás. Ha nem teljesül a feltétel, akkor BookingTimeConflictException generálódik, és az alkalmazás 400-as Status kóddal tér vissza.
3. Foglalás akkor lehetséges, ha a hajó befér a kívánt kikötőhelyre. Ha nem teljesül a feltétel, akkor BoatSizeException generálódik, és az alkalmazás 400-as Status kóddal tér vissza.
...
Az alkalmazáshoz a SWAGGER UI biztosít API dokumentációt, és lehetőséget a webszolgáltatások kipróbálásához.
